...
public class SearchOffenseController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        if (offenseByName == null) {
            // Offense not found, show alert
            Platform.runLater(() -> {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setTitle("Offense Not Found");
                alert.setHeaderText(null);
                alert.setContentText("The offense with name '" + offenseName + "' was not found.");
                alert.showAndWait();
            });
        } else {
            ...
            TableColumn<Offense, String> offenseTypeColumn = new TableColumn<>("OFFENSE TYPE");
            offenseTypeColumn.setCellValueFactory(new PropertyValueFactory<>("type"));
            offenseTypeColumn.getStyleClass().addAll("type-column");

            TableColumn<Offense, String> offenseDescriptionColumn = new TableColumn<>("OFFENSE DESCRIPTION");
